...
//:::::::::::::::::::::::::::::::::::::::::
// Basic Start/End timer support for Defrag
void Use_target_startTimer(gentity_t* self, gentity_t* other, gentity_t* activator) {
	...
	// Skip cases
	if (!cl) {
		return;  // Activator is not a client
	}
	if (cl->ps.pm_type != PM_NORMAL) {
		return;  // Client is not in normal movement mode
	}
	if (cl->ps.stats[STAT_HEALTH] <= 0) {
		return;  // Client is not alive
	}
